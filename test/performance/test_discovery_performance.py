...
def check_service_availability() -> Tuple[bool, bool]:
    ...
    try:
        response = httpx.get(
            f"{SOVEREIGN_URL}/deepcheck", timeout=SERVICE_CHECK_TIMEOUT
        )
...
class TestDiscoveryPerformance:
    ...
    def reset_context(self, context_value: str = "helloworld") -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Warning: Context reset returned status code {response.status_code}"
                )
        except httpx.RequestError as e:
    ...
    def update_context(self, new_value: str) -> float:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Warning: Context update returned status code {response.status_code}"
                )
            ...
            if verify_response.status_code == 200:
                ...
                if current_context != new_value:
                    print(
                        "WARNING: Context in mock service doesn't match requested value!"
                    )
            else:
                print(
                    f"Warning: Failed to verify context update, status code {verify_response.status_code}"
                )
        except httpx.RequestError as e:
    ...
    def check_context_propagation(self, expected_value: str) -> Tuple[bool, float]:
        ...
        start_time = time.time()
        max_retries = (
            int(MAX_WAIT_TIME / RETRY_DELAY) + 1
        )  # Calculate max retries based on timeout and delay
        last_message_length = 0

        # Print initial waiting message
        print(
            f"Waiting for context '{expected_value}' to propagate (timeout: {MAX_WAIT_TIME}s, retries: {max_retries})...",
            end="",
            flush=True,
        )
        ...
        for attempt in range(1, max_retries + 1):
            ...
            if elapsed_time > MAX_WAIT_TIME:
                ...
                print(
                    f"Timeout after {elapsed_time:.1f} seconds waiting for context '{expected_value}'"
                )
                return False, elapsed_time

            try:
                response = httpx.get(
                    f"{SOVEREIGN_URL}/ui/resources/listeners/port80", timeout=5.0
                )
                if response.status_code == 200:
                    data = response.json()
                    try:
                        actual_value = data["resources"][0]["filter_chains"][0][
                            "filters"
                        ][0]["typed_config"]["stat_prefix"]
                        if actual_value == expected_value:
                            elapsed_time = time.time() - start_time
                            # Clear the current line if using terminal escape codes
                            if USE_TERMINAL_ESCAPE_CODES:
                                print(
                                    "\r" + " " * last_message_length + "\r",
                                    end="",
                                    flush=True,
                                )
                            print(
                                f"Context '{expected_value}' found after {elapsed_time:.3f}s ({attempt} attempts)"
                            )
                            ...
                            if USE_TERMINAL_ESCAPE_CODES:
                                ...
                                print(
                                    f"Attempt {attempt}: Found value '{actual_value}', expecting '{expected_value}'"
                                )
                        ...
                        if USE_TERMINAL_ESCAPE_CODES:
                            print(
                                "\r" + " " * last_message_length + "\r" + status_msg,
                                end="",
                                flush=True,
                            )
                            last_message_length = len(status_msg)
                            ...
                            print(
                                f"Attempt {attempt}: Error extracting value: {str(e)}"
                            )
                else:
                    # Update the status on the same line if using terminal escape codes
    ...
    def test_context_update_performance(self) -> None:
        ...
        for iteration in range(NUM_ITERATIONS):
            ...
            for context_value in CONTEXT_VALUES:
                ...
                success, propagation_time = self.check_context_propagation(
                    context_value
                )
                ...
                if not success:
                    print(
                        f"WARNING: Context '{context_value}' was not propagated after {propagation_time:.3f}s"
                    )
            ...
            if iteration_results:  # Only add if we have results
                results.append(
                    {"iteration": iteration + 1, "results": iteration_results}
                )
        ...
        update_times = [
            r["update_time"] for iteration in results for r in iteration["results"]
        ]
        propagation_times = [
            r["propagation_time"] for iteration in results for r in iteration["results"]
        ]
        total_times = [
            r["total_time"] for iteration in results for r in iteration["results"]
        ]

        print("\nPerformance Statistics:")
        print(
            f"Update Time (s): min={min(update_times):.3f}, max={max(update_times):.3f}, "
            f"mean={statistics.mean(update_times):.3f}, median={statistics.median(update_times):.3f}"
        )
        print(
            f"Propagation Time (s): min={min(propagation_times):.3f}, max={max(propagation_times):.3f}, "
            f"mean={statistics.mean(propagation_times):.3f}, median={statistics.median(propagation_times):.3f}"
        )
        print(
            f"Total Time (s): min={min(total_times):.3f}, max={max(total_times):.3f}, "
...
class TestDiscoveryRequestPerformance:
    ...
    def make_discovery_request(
        self, request_data: Dict[str, Any], resource_type: str
        ...
        try:
            ...
            if response.status_code != 200:
                print(
                    f"Warning: Discovery request returned status code {response.status_code}"
                )
                return {
    ...
    def test_listeners_discovery_performance(
        ...
        print(
            f"Time (s): min={min(times):.3f}, max={max(times):.3f}, "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--skip-checks", action="store_true", help="Skip service availability checks"
    )
    parser.add_argument(
        "--single-line",
        action="store_true",
