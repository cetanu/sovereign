...
* Context
    ...
    * It uses a task event loop to process context efficiently and on-schedule
    * Each `Loadable` can now have its own retry policy and schedule.

```yaml
template_context:
  # These will act as defaults in case the tasks below have none
  refresh: true
  refresh_rate:
  refresh_cron: "*/2 * * * *"
  context:
    foobar:
      # The following fields are aliases for protocol and serialization respectively
      loader: https
      deserialize_with: json
      path: foobar.com/api/v2/data
      interval: 300
      retry_policy:
        num_retries: 2
        interval: 5 # between retries
```

* UI changes
    * The dropdown to select service-cluster is removed
